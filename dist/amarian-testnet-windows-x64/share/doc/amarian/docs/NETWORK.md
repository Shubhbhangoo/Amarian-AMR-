...
**The message framing, handshake, peer manager, headers-first sync, block download path, inventory block relay, transaction relay, connection caps, ping timeout, bounded address exchange, persistent address storage, and temporary endpoint bans are implemented.**
...
The original design note below predates the Phase 4 implementation. The live status is
given at the top of this document; the current implementation is built as the `amarian_net`
target and uses Asio for the dedicated peer event loop.
